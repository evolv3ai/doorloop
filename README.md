...
const response = await client.users.retrieve('userId').asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: user, response: raw } = await client.users.retrieve('userId').withResponse();
...
1. Via the `DOORLOOP_LOG` environment variable
...
const client = new Doorloop({
  logger: logger.child({ name: 'Doorloop' }),
...
const client = new Doorloop({ fetch });
```
...
```ts
import Doorloop from 'npm:doorloop';
