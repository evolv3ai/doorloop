...
```sh
git clone git@github.com:stainless-sdks/doorloop-typescript.git
cd doorloop-typescript
./scripts/bootstrap
./scripts/build
```
...
> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npx -y doorloop-mcp`
...
```json
{
  "mcpServers": {
    "doorloop_api": {
      "command": "node",
      "args": [
        "/path/to/local/doorloop-typescript/packages/mcp-server",
        "--client=claude",
        "--tools=dynamic"
      ],
      "env": {
